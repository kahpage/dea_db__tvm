...
def main():
    ...
    raw_url = "https://web.archive.org/web/20120627164312id_/http://ketto.com/mimiken/alist.cgi?mi=79&sf=&spf=1"
    ...
    for table in tables:
        ...
        for row in table_rows:  # Skip header row
                ...
                description_parts = []
                desc_spaces = sanitize_string(cols[5].get_text(strip=True))
                if desc_spaces:
                    description_parts.append(f"Spaces: {desc_spaces}")
                ...
                tweeter_tag = cols[3].select_one("a")
                if tweeter_tag and tweeter_tag.has_attr("href"):
                    circle_urls.append(tweeter_tag["href"])

                circle = Circle(
                    aliases=[circle_name],
                    pen_names=[circle_pen_name] if circle_pen_name else None,
                    links=[url.replace("https://web.archive.org/web/20120627164312/", "") for url in circle_urls] if circle_urls else None,
                    position=position,
